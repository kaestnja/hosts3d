...
The F9 switch-topology scene starts the SCALANCE helper with the built-in lab default for `sw6248xr328` (`192.168.6.248`, SNMPv2c, read-only defaults `private` then `public`) and updates `.hosts3d/switch-topology.txt` for display. `.hosts3d/switches.txt` is only an optional human-editable override for another switch address or non-default SNMP data. With `auto_refresh=1`, an enabled override line refreshes automatically; `View > Refresh Switch Topology` starts the same refresh manually. For non-default SNMPv1/v2c community values use `community=...` directly; for SNMPv3 passwords prefer `user_env`, `auth_pass_env`, and `priv_pass_env`.
