...
The F9 switch-topology scene starts the SCALANCE refresh runner with the built-in lab default for `sw6248xr328` (`192.168.6.248`, SNMPv2c, read-only defaults `private` then `public`) when no enabled switch rows exist, and updates `.hosts3d/switch-topology.txt` for display. `.hosts3d/switches.txt` is the optional human-editable switch inventory for other switches or non-default SNMP data. With `auto_refresh=1`, enabled rows refresh automatically; `View > Refresh Switch Topology` starts the same serial refresh manually. Per-switch raw JSON is written below `.hosts3d/snmp/`. For non-default SNMPv1/v2c community values use `community=...` directly; for SNMPv3 passwords prefer `user_env`, `auth_pass_env`, and `priv_pass_env`. Shared SNMP helper documentation lives in `Tools/snmp/README.md`.
